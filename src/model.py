...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1, feedforward_expansion=3,is_decoder=False):
...
class Transformer(nn.Module):
    def __init__(self,vocab_size,embed_dim, heads, blocks,device ,dropout=0.1, ff_expansion = 3) -> None:
        ...
        self.transformer_encoder = nn.ModuleList([
            TransformerBlock(embed_dim, heads, dropout, ff_expansion) for _ in range(blocks)
        ])
        self.transformer_decoder = nn.ModuleList([
            TransformerBlock(embed_dim, heads, dropout, ff_expansion, is_decoder=True) for _ in range(blocks)
        ])
        self.dropout = nn.Dropout(dropout)
    ...
    def make_trg_mask(self, trg):
        ...
        tril = torch.tril(torch.ones((trg_len, trg_len), dtype=torch.bool, device=self.device)).expand(N, 1, trg_len, trg_len)
