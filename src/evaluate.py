...
def get_args():
    ...
    parser.add_argument("--test_samples", type=int, default=100, help="Số lượng câu test (đừng test hết nếu dùng Gemini)")
...
def beam_search_decode(model, src, sp, device, beam_size=3, max_len=128):
    ...
    for _ in range(max_len):
        ...
        k_candidates = sorted(new_candidates, key=lambda x: x[1], reverse=True)[:beam_size]
...
def main():
    ...
    for item in tqdm(dataset):
        ...
        src_tensor = torch.tensor(src_ids, dtype=torch.long).unsqueeze(0).to(args.device)
        
        # --- BEAM SEARCH ---
        pred_ids = beam_search_decode(model, src_tensor, sp, args.device, beam_size=args.beam_size, max_len=args.max_len)
    ...
    for i, length in enumerate(sent_lens):
        ...
        if bin_idx not in bins: bins[bin_idx] = {'refs': [], 'cands': []}
