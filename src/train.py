...
def get_args():
    ...
    parser.add_argument("--vocab_size", type=int, default=8000, help="Vocab size (khớp với SPM)")
...
def train_one_epoch(model, loader, optimizer, criterion, scaler, device, epoch):
    ...
    for batch in loop:
        ...
        # optimizer.zero_grad() ở đây hoặc cuối loop đều được
        optimizer.zero_grad()
        
        with torch.amp.autocast_mode.autocast(enabled=(device == "cuda"), device_type=device):
...
def main():
    ...
    print(f"Test Parameter Count: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    for epoch in range(1, args.epochs + 1):
        # Train
        train_loss = train_one_epoch(model, train_loader, optimizer, criterion, scaler, args.device, epoch)
        # Validate
