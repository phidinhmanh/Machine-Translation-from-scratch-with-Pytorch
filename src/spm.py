...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train a SentencePiece model from a Python script."
    )

    # Định nghĩa các tham số tương tự như spm_train
    parser.add_argument("--input", type=str, required=True, help="Input text file.")
    parser.add_argument(
        "--model_prefix", type=str, required=True, help="Prefix for the model name."
    )
    parser.add_argument(
        "--vocab_size", type=int, required=True, help="Vocabulary size."
    )
    parser.add_argument(
        "--character_coverage", type=float, default=1.0, help="Character coverage."
    )
    parser.add_argument(
        "--model_type",
        type=str,
        default="unigram",
        help="Model type (e.g., unigram, bpe).",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    print(
        f"Training complete. Model and vocab files are saved in '{args.output_dir}' with prefix '{args.model_prefix}'."
    )
